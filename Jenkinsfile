node {
 ...
 stage ("Deploy Application on Azure Kubernetes Service")
 {
  ...
  case "release":
        ...
        withCredentials([usernamePassword(credentialsId: 'kama-kama', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
          sh "sudo kubectl --kubeconfig ~jenkinsdemo5/.kube/config -n stage get secret kama-kama2 ||  sudo kubectl --kubeconfig ~jenkinsdemo5/.kube/config --namespace=stage create secret docker-registry kama-kama2 --docker-server ${acr} --docker-username $USERNAME --docker-password $PASSWORD"
        }
