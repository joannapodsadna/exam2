 ...
 stage ("Deploy Application on Azure Kubernetes Service")
 {
  ...
  case "release":
    ...
    case "dev":
        ...
        withCredentials([usernamePassword(credentialsId: 'kama-kama', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
          sh "kubectl  -n dev get secret kama-kama3 || kubectl  --namespace=dev create secret docker-registry kama-kama3 --docker-server ${acr} --docker-username $USERNAME --docker-password $PASSWORD"
        }
